...
def build_word_index():
    ...
    if not os.path.exists(src_vocab_file):
        with open(src_vocab_file, 'w', encoding='utf-8') as source:
            ...
            for line in f:
                values = line.split()
                word = values[0]  # 取词
                # if type(word) is np.unicode:
                if isinstance(word, np.unicode):
    ...
    if not os.path.exists(tgt_vocab_file):
        with open(src_vocab_file, 'r', encoding='utf-8') as source:
            dict_word = {}
...
def load_word2vec_embedding(vocab_size):
    ...
    with open(word_embedding_file, 'r', encoding="utf-8") as f:
        line = f.readline() # 第一行是（词的个数，词的维数）
        line = f.readline()
        while line:
            values = line.split()
            try:
